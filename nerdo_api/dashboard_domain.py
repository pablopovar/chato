...
from .share_sessions import MAX_DURATION_HOURS, MIN_DURATION_HOURS, ShareStore
    ...
    for path in sorted(_users_dir().glob("*/*/nerdo.json")):
        try:
            ...
            domain = _normalize_domain(
                str(raw.get("domain") or path.parent.name)
            )
        except Exception:
            continue
        rows.append(
...
def _validated_update(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if not model or len(model) > 300:
        raise HTTPException(
            400,
            "model is required and must be 300 characters or fewer.",
        )
...
def _chat(
    ...
    if not 2 <= len(clean_question) <= 4000:
        raise HTTPException(
            400,
            "question must contain between 2 and 4,000 characters.",
        )
...
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Nerdo domain dashboard</title><style>
:root{--navy:#00043a;--red:#ff002b;--line:#d9dce5;--muted:#5a6076;--soft:#eef1f7;--bg:#f3f4f7}*{box-sizing:border-box}body{margin:0;font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:var(--bg);color:var(--navy)}button,select,textarea,input{font:inherit}header{padding:22px 5vw;background:var(--navy);color:#fff}.head{display:flex;justify-content:space-between;gap:20px;align-items:end}h1{margin:0;font-size:36px}header p{margin:6px 0 0;color:#d8dbea}header a{color:#fff;text-decoration:none;font-weight:800}main{padding:24px 5vw}.tabs{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px}.tabs button{background:var(--soft);color:var(--navy)}.tabs button.active{background:var(--navy);color:#fff}.panel{background:#fff;border:1px solid var(--line);border-radius:12px;padding:22px}.hidden{display:none}label{display:block;font-size:12px;text-transform:uppercase;letter-spacing:.08em;color:var(--muted);font-weight:800;margin-bottom:8px}select,textarea,input{width:100%;border:1px solid var(--line);border-radius:8px;padding:11px;color:var(--navy);background:#fff}textarea{resize:vertical}.prompt{min-height:280px}.question{min-height:130px}.grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px}.wide{grid-column:1/-1}.actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:16px}button{border:0;border-radius:8px;padding:10px 14px;font-weight:800;cursor:pointer}.primary{background:var(--red);color:#fff}.secondary{background:var(--soft);color:var(--navy)}button:disabled{background:#c8cbd5;color:#fff}.notice{min-height:24px;font-weight:700}.help{font-size:13px;color:var(--muted);margin-top:7px}.answer{white-space:pre-wrap;line-height:1.55;border:1px solid var(--line);border-radius:8px;padding:16px;margin-top:16px}.debug{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px;margin-top:12px}.debug div{background:var(--soft);border-radius:8px;padding:9px}.debug dt{font-size:10px;text-transform:uppercase;color:var(--muted);font-weight:800}.debug dd{margin:4px 0 0;overflow-wrap:anywhere}details{border:1px solid var(--line);border-radius:8px;padding:10px 12px;margin-top:10px}.source{padding:10px 0;border-top:1px solid var(--line)}.source:first-of-type{border-top:0}.source pre{white-space:pre-wrap;font:inherit}.history{display:grid;grid-template-columns:minmax(280px,380px) minmax(0,1fr);gap:16px}.conversation-list{border:1px solid var(--line);border-radius:10px;overflow:auto;max-height:720px}.conversation{display:block;width:100%;text-align:left;background:#fff;color:var(--navy);border-radius:0;border-bottom:1px solid var(--line);padding:13px}.conversation:hover,.conversation.active{background:var(--soft)}.conversation:last-child{border-bottom:0}.conversation time{display:block;font-size:12px;color:var(--muted);margin-top:5px}.preview{font-size:13px;color:var(--muted);margin-top:7px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.transcript{border:1px solid var(--line);border-radius:10px;padding:16px;min-height:300px}.message{margin-bottom:14px;padding:12px;border-radius:9px;white-space:pre-wrap;line-height:1.5}.message.user{background:var(--soft)}.message.assistant{border:1px solid var(--line)}.role{font-size:11px;text-transform:uppercase;letter-spacing:.08em;font-weight:850;color:var(--muted);margin-bottom:6px}.empty{color:var(--muted);padding:18px}.share-box{max-width:720px}.share-output{margin-top:18px;border:1px solid var(--line);border-radius:10px;padding:16px;background:var(--soft)}.share-output a{overflow-wrap:anywhere;font-weight:800;color:var(--navy)}@media(max-width:820px){.grid,.debug,.history{grid-template-columns:1fr}.head{display:block}.head a{display:inline-block;margin-top:12px}}
</style></head><body><header><div class="head"><div><h1 id="title">Domain dashboard</h1><p>Model configuration, testing, history, and client sessions.</p></div><a href="/dashboard/">All domains</a></div></header><main>
<div class="tabs"><button class="active" data-tab="config">Configuration</button><button data-tab="chat">Test chat</button><button data-tab="history">Past chats</button><button data-tab="share">Session Share</button></div><p id="notice" class="notice"></p>
<section id="configPanel" class="panel"><div class="grid"><label class="wide">Default model<select id="model"></select><span id="modelHelp" class="help"></span></label><label class="wide">System prompt<textarea id="systemPrompt" class="prompt" maxlength="20000"></textarea></label><label>Temperature<input id="temperature" type="number" min="0" max="2" step="0.05"></label><label>Maximum output tokens<input id="maxTokens" type="number" min="64" max="8192" step="1"></label><label>Retrieval results<input id="maxResults" type="number" min="1" max="12" step="1"></label><label>Maximum context characters<input id="maxContextChars" type="number" min="2000" max="100000" step="500"></label></div><div class="actions"><button id="save" class="primary">Save configuration</button><button id="reload" class="secondary">Discard changes</button></div></section>
...
<section id="sharePanel" class="panel hidden"><div class="share-box"><h2>Create a client session</h2><p>How long should the claimed session remain available?</p><label for="shareHours">Session duration in hours<input id="shareHours" type="number" min="1" max="8760" step="1" value="48"></label><p class="help">The link can claim one browser session only. Its expiry clock starts when it is first opened successfully.</p><div class="actions"><button id="createShare" class="primary">Create one-time share link</button></div><div id="shareOutput"></div></div></section>
</main><script>
const pathParts=location.pathname.split('/').filter(Boolean);const domain=decodeURIComponent(pathParts[pathParts.length-1]);let sessionId=null;let historyLoaded=false;const $=s=>document.querySelector(s),esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const notice=$('#notice');$('#title').textContent=domain;
function setTab(tab){document.querySelectorAll('[data-tab]').forEach(b=>b.classList.toggle('active',b.dataset.tab===tab));$('#configPanel').classList.toggle('hidden',tab!=='config');$('#chatPanel').classList.toggle('hidden',tab!=='chat');$('#historyPanel').classList.toggle('hidden',tab!=='history');$('#sharePanel').classList.toggle('hidden',tab!=='share');if(tab==='history'&&!historyLoaded)loadHistory()}
...
$('#reload').addEventListener('click',()=>loadConfig().catch(e=>notice.textContent=e.message));$('#newSession').addEventListener('click',()=>{sessionId=null;$('#result').innerHTML='';notice.textContent='New session.'});
...
$('#conversationList').addEventListener('click',async e=>{const b=e.target.closest('[data-session]');if(!b)return;document.querySelectorAll('.conversation').forEach(x=>x.classList.toggle('active',x===b));$('#transcript').innerHTML='<div class="empty">Loading transcript…</div>';const r=await fetch(`/dashboard/api/domains/${encodeURIComponent(domain)}/conversations/${encodeURIComponent(b.dataset.session)}`,{credentials:'same-origin'});const p=await r.json();if(!r.ok){$('#transcript').innerHTML=`<div class="empty">${esc(p.detail||`HTTP ${r.status}`)}</div>`;return}$('#transcript').innerHTML=(p.messages||[]).map(x=>`<div class="message ${esc(x.role)}"><div class="role">${esc(x.role)}</div>${esc(x.content)}</div>`).join('')||'<div class="empty">No messages.</div>'});
$('#createShare').addEventListener('click',async()=>{const hours=Number($('#shareHours').value);$('#createShare').disabled=true;notice.textContent='Creating one-time share link…';try{const r=await fetch(`/dashboard/api/domains/${encodeURIComponent(domain)}/session-shares`,{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({hours})});const p=await r.json();if(!r.ok)throw Error(p.detail||`HTTP ${r.status}`);$('#shareOutput').innerHTML=`<div class="share-output"><div><strong>One-time share link</strong></div><p><a id="shareLink" href="${esc(p.share_url)}" target="_blank" rel="noopener">${esc(p.share_url)}</a></p><p>After it is claimed once, this link cannot open another session. The claimed session lasts ${esc(p.duration_hours)} hours.</p><button id="copyShare" class="secondary">Copy link</button></div>`;$('#copyShare').addEventListener('click',()=>navigator.clipboard.writeText(p.share_url));notice.textContent=''}catch(e){notice.textContent=e.message}finally{$('#createShare').disabled=false}});
loadConfig().catch(e=>notice.textContent=e.message);
</script></body></html>'''


def install_dashboard_domain(app: FastAPI, settings: Settings) -> None:
    share_store = ShareStore(settings.database_path)
    ...
    def create_share(
        ...
        try:
            ...
            raise HTTPException(400, "hours must be an integer.") from exc
        try:
            record, claim_token = share_store.create(normalized, hours)
        except ValueError as exc:
        ...
        return {
            ...
            "single_use": True,
            "expires_after_claim": True,
            "duration_range": {
                "minimum_hours": MIN_DURATION_HOURS,
                "maximum_hours": MAX_DURATION_HOURS,
            },
        }
    ...
    app.add_api_route(
        "/dashboard/api/domains/{domain}/session-shares",
        create_share,
        methods=["POST"],
        include_in_schema=False,
    )
